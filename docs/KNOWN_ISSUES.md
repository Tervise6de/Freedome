...
| The mower's handlebar was two disconnected sticks and a floating bar. Each tube was rotated +52 degrees about X instead of -34, so it ran from a point 856 mm behind the machine on the floor up to a point over the deck - touching neither the mower nor the cross grip it was supposed to hold up. The sign of a rotation about X has now caught out the door leaf, the ridge cap and this | Working out where the handle actually went, while adding the starter | Tube length and lean derive from `MowerHandleFoot` and `MowerHandleGrip`, and `TheMowerHandleJoinsTheDeckToTheGrips` reconstructs the tube and checks both ends land on them. Flipping the sign back fails it |
| Three screwdrivers hung on the pegboard as geometry, not as carryables, for the whole of the escape game. The player could see three of the tool they needed on a wall and pick up none of them, while the only real one was shut in a drawer. Nothing about the route is signposted, but the room should not actively lie either | Reading the tool rack while adding carryables | Replaced with three bare hooks, which is the most ordinary thing on a pegboard. The pliers and the try square stay |
| The bench cupboard carcass had sides and a back and no bottom, so anything put inside it would have fallen through to the floor. Nobody had noticed because nothing had ever been put in it and the doors did not open | Adding the cupboard contents | Bottom board added; `CupboardShelfY` derives from it and the placement test checks against it |
| The light probe lattice stopped 400 mm short of every wall and started 250 mm above the floor. Probe interpolation is only defined inside the convex hull of the probes, so every movable object resting on the floor, and everything within 400 mm of a wall, sampled whatever the nearest outer tetrahedron happened to hold - including three of the five loose objects at their starting positions. That is lighting going wrong specifically near walls, and a pop as a carried object crosses the boundary | Reported as "lighting is weird next to walls", then found by reading `BuildLightProbes` against the room's own half-extents | Lattice extended to 60 mm inside each lining and down to y = 0.05, 5 x 7 x 5. `ProbeHull()` reports the box and `TheProbeHullReachesTheWallsAndTheFloor` checks the room and every carryable against it |
| The drawer fronts and the cupboard doors met the bench carcass edge to edge, in the same timber, on a carcass with no face frame - so the drawers read as lines drawn on the bench rather than as parts of it. The bench top was also exactly coplanar with the wall lining, giving no shadow line where the two met | Reported as "a drawer blends in the desk" | Face frame on both the drawer bank and the cupboard - stiles, rails, and a 3 mm reveal round every front - a harder chamfer on the moving parts than on the frame, and the bench top held 8 mm off the wall |
